    ...
    def inserir():
        ...
        try:
            with st.container(border=True):
                ...
                valor = colun2.number_input("Valor (R$): ", min_value=0.0, step=0.01, format="%.2f")
                metodo_pagamento = colun1.selectbox("Método de Pagamento: ", m, format_func=lambda metodo: metodo.nome)
                categoria = colun2.selectbox("Categoria: ", c, format_func=lambda categoria: categoria.nome)
                data = colun1.date_input("Data: ", format="DD/MM/YYYY", max_value='today')
                st.write('---')
                if st.button("Cadastrar", type='primary'):
                    View.despesa_inserir(descricao, valor, categoria.id, st.session_state.usr.id, metodo_pagamento.id, data)
    ...
    def atualizar():
        ...
        if despesa:
            with st.container(border=True):
                ...
                novo_valor = colun2.number_input("Novo Valor (R$):", value=despesa.valor, min_value=0.0, step=0.01, format="%.2f")
                ...
                nova_categoria = colun2.selectbox("Nova Categoria:", categoria, index=[c.id for c in categoria].index(despesa.categoria_id), format_func=lambda categoria: categoria.nome)
                data = colun1.date_input("Nova Data: ", format="DD/MM/YYYY", max_value='today', value=despesa.data)

                st.write('---')
                if st.button("Atualizar", type='primary'):
                    View.despesa_atualizar(despesa.id, nova_descricao, novo_valor, nova_categoria.id, despesa.usuario_id, novo_metodo_pagamento.id, data)
